...
namespace io
{
    ...
    void printTable(std::vector<std::pair<long double, long double>> table, std::string header1, std::string header2,
                    int indentWidth, int count)
    {
        return printTable(
            2, std::min<int>(table.size(), count), {header1, header2},
            [&table](int row, int col) { return col ? table[row].second : table[row].first; }, indentWidth);
    }

    void printTable(std::vector<std::vector<long double>> table, std::vector<std::string> headers, int indentWidth)
    {

        int width = headers.size(), height = table.size();
        for (auto &&row : table) {
            assert(row.size() == width || "Invalid table");
        }

        printTable(
            width, height, headers, [&table](int row, int col) { return table[row][col]; }, indentWidth);
    }

    void printTable(int width, int height, std::vector<std::string> headers, std::function<long double(int, int)> getElement,
                    int indentWidth)
    {
        std::cout << std::right;
    ...
    void printNodesAndCoefs(const std::vector<std::pair<long double, long double>> &coefs, int startFrom, std::string variable,
                            std::string coef)
    {
        for (int i = 0, s = coefs.size(); i < s; i++) {
